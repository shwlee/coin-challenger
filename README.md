...
## 그런데 아직까지는 그냥 샘플로만 실행되는 거예요.

게임을 진행하기 위해서는 여러분의 알고리즘 코드가 필요합니다.(이하 `플레이어 코드` 라고 부릅니다.)

알고리즘 코드는 C#, javascript 로 작성 가능합니다.   

각 샘플 코드는 알고리즘 작성 템플릿 같은 거라고 보면 됩니다.

샘플 껍딱 위에 여러분의 알고리즘을 작성해 주세요.
...
구조에 대한 설명은 `C#` 기준으로 먼저 설명합니다.    
*(각 게임 플랫폼에 대한 반복 설명은 생략합니다.)*
...
플레이어 코드 파일을 읽은 후 각 플레이어 호스트에 게임 정보를 전달합니다.
...
이후 게임 프로세스는 플레이어의 이름을 확인하기 위해  `GetName()` 를 호출합니다. 
...
플레이어 코드는 이번 턴에서 플레이어가 이동할 방향을 결정합니다.    
